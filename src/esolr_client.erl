...
handle_call({search,Query,Options},From,State=#esolr{search_url=URL,pending=P,search_timeout=Timeout}) ->	
    ...
    Pendings = gb_trees:insert(RequestId,{From,{search,Options}},P),
...
handle_http_response({{_HttpV,200,_Reason},_Headers,Data},{search,Options},Client) ->
    case proplists:get_value(return, Options, default) of
        default ->
            {struct,Response} = mochijson2:decode(Data),
            {value,{<<"responseHeader">>,{struct,Headers}},RestResponse} = lists:keytake(<<"responseHeader">>,1,Response),
        ...
        raw ->
            gen_server:reply(Client,{ok,Data})
    end;
...
parse_search_response(Response,Client) ->
    {value,{<<"response">>,{struct,SearchRespFields}},RestResponse} = lists:keytake(<<"response">>,1, Response),
    {value,{<<"docs">>,Docs},RespFields} =  lists:keytake(<<"docs">>,1,SearchRespFields),
    gen_server:reply(Client,{ok,RespFields,[{doc,DocFields} || {struct,DocFields}<-Docs],RestResponse}).
...
encode_search_option({facet_date, Field, Start, End, Gap}) ->
    ["facet=on&",
     "facet.date=",url_encode(Field), 
     "&facet.date.start=", Start,
     "&facet.date.end=", End,
     "&facet.date.gap=", url_encode(Gap)].
